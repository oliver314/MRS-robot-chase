...
def sample_random_position(occupancy_grid):
  ...
  while True:
    if np.random.rand() > 0.5:
      ...
      position = np.random.normal([0.3, 0.2], 0.6, 2)
...
def rrt(start_pose, goal_position, occupancy_grid):
  ...
  for _ in range(MAX_ITERATIONS): 
    ...
    if np.linalg.norm(v.position - goal_position) < .2:
      ...
      # Deciding factor whether to use Rapidly Exploring Forest or Tree
      #break

  # find node that has the shortest path:
  min_length = np.inf
  best_parent = None
  for final_node in final_nodes:
    ...
    while v.parent is not None:
      # Add path length
      center, radius = find_circle(v.parent, v)
      theta1 = np.arctan2(v.parent.pose[1] - center[1], v.parent.pose[0] - center[0])
      theta2 = np.arctan2(v.pose[1] - center[1], v.pose[0] - center[0])
      delta_theta = min((theta2 - theta1) % (2*np.pi), (theta1-theta2) % (2*np.pi)) 
      length += radius * delta_theta
      v = v.parent
    if length < min_length:
      best_parent = final_node
      min_length = length
  return start_node, best_parent, final_nodes, min_length
...
def draw_solution(start_node, final_node=None, final_nodes=[]):
  ax = plt.gca()
  ...
  for one_final_node in final_nodes:
    plt.scatter(one_final_node.position[0], one_final_node.position[1], s=10, marker='o', color='k')
    # Draw final path.
    v = one_final_node
    while v.parent is not None:
      draw_path(v.parent, v, color='r', lw=2)
      v = v.parent
...
if __name__ == '__main__':
  ...
  if eval:
    length = np.zeros(100)
    for i in range(100):
      start_node, final_node, final_nodes, final_cost = rrt(START_POSE, GOAL_POSITION, occupancy_grid)
      length[i] = final_cost
    print("Improved Mean length " + str(np.mean(length)) + " with std " + str(np.std(length)) + " with median " + str(np.median(length))) 
  else:
    start_node, final_node, final_nodes, final_cost = rrt(START_POSE, GOAL_POSITION, occupancy_grid)
  ...
  draw_solution(start_node, final_node, final_nodes)
