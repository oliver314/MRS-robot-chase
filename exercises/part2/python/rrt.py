...
def rrt(start_pose, goal_position, occupancy_grid):
  ...
  # Evaluate length for performance comparison with the improved version
  length = 0
  ...
  return start_node, final_node, length
...
if __name__ == '__main__':
  ...
  if eval:
    length = np.zeros(100)
    for i in range(100):
      start_node, final_node, final_cost = rrt(START_POSE, GOAL_POSITION, occupancy_grid)
      length[i] = final_cost
    print("Standard Mean length " + str(np.mean(length)) + " with std " + str(np.std(length)) + " with median " + str(np.median(length))) 
  else:
    start_node, final_node, final_cost = rrt(START_POSE, GOAL_POSITION, occupancy_grid)
