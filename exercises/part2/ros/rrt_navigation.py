...
def run(args):
  ...
  path_past_publisher = rospy.Publisher('/path_past', Path, queue_size=1)
  slam = SLAM()
  goal = GoalPose()
  frame_id = 0
  current_path = []
  ...
  while not rospy.is_shutdown():
    ...
    past_path.append([slam.pose[X], slam.pose[Y]])
    # Publish past path to RViz.
    path_past_msg = Path()
    path_past_msg.header.seq = frame_id
    path_past_msg.header.stamp = rospy.Time.now()
    path_past_msg.header.frame_id = 'map'
    for u in past_path:
      pose_msg = PoseStamped()
      pose_msg.header.seq = frame_id
      pose_msg.header.stamp = path_past_msg.header.stamp
      pose_msg.header.frame_id = 'map'
      pose_msg.pose.position.x = u[X]
      pose_msg.pose.position.y = u[Y]
      path_past_msg.poses.append(pose_msg)
    path_past_publisher.publish(path_past_msg)
    ...
    # Run RRT.
    start_node, final_node, length = rrt.rrt(slam.pose, goal.position, slam.occupancy_grid)
