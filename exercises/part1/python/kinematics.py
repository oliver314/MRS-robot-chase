...
def rk4(current_pose, t, dt):
  ...
  theta_n = current_pose[YAW]
  theta_n_plus_1 = current_pose[YAW] + 1/6.0 * dt * (k_th(t) + 4 * k_th(t+dt/2) + k_th(t+dt))
  theta_n_plus_half = current_pose[YAW] + 1/6.0 * dt/2.0 * (k_th(t) + 4 * k_th(t+dt/4) + k_th(t+dt/2))
  # Pose at time t+dt
  ...
  next_pose[YAW] = theta_n_plus_1 # or analytically: np.sin(t+dt)
