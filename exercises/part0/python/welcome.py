...
def finished_tictactoe(state):
  ...
  convert = lambda x: -1 if x=='X' else convert2(x)
  convert2 = lambda x: 1 if x=='O' else 0
  state = np.array([[convert(s) for s in line] for line in state])
  return any(np.abs(np.sum(state, axis=0))==3) or any(np.abs(np.sum(state, axis=1))==3) or \
     abs(state[0, 0]+state[1,1]+state[2,2]) == 3 or abs(state[0, 2]+state[1,1]+state[2,0]) == 3
